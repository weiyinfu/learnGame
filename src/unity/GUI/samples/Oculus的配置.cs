  ...
  class GUIHelper {
    ...
    public class OculusPlatformSettingsEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUIContent riftAppIDLabel = new GUIContent("Oculus Rift [?]",
                "This AppID will be used when building to the Windows target.");
            ...
            {
              EditorGUILayout.HelpBox("Please enter a valid Oculus Go/Quest or Gear VR App ID.", UnityEditor.MessageType.Error);
            }
            else
            {
              var msg = "Configured to connect with App ID " + PlatformSettings.MobileAppID;
              EditorGUILayout.HelpBox(msg, UnityEditor.MessageType.Info);
            }
            ...
            if (isUnityEditorSettingsExpanded)
            {
                GUIHelper.HInset(6, () =>
                {
                    bool HasTestAccessToken =
                        !String.IsNullOrEmpty(StandalonePlatformSettings.OculusPlatformTestUserAccessToken);
                    if (PlatformSettings.UseStandalonePlatform)
                    {
                        ...
                        {
                            EditorGUILayout.HelpBox("Please enter a valid user credentials.", UnityEditor.MessageType.Error);
                        }
                        else
                        {
                            var msg =
                                "The Unity editor will use the supplied test user credentials and operate in standalone mode.  Some user data will be mocked.";
                            EditorGUILayout.HelpBox(msg, UnityEditor.MessageType.Info);
                        }
                    }
                    else
                    ...
                    PlatformSettings.UseStandalonePlatform = MakeToggle(new GUIContent(useStandaloneLabel, useStandaloneHint),
                    ...
                    if (!HasTestAccessToken)
                    {
                        ...
                        StandalonePlatformSettings.OculusPlatformTestUserEmail =
                            MakeTextBox(new GUIContent(emailLabel, emailHint),
                                StandalonePlatformSettings.OculusPlatformTestUserEmail);
                        ...
                        StandalonePlatformSettings.OculusPlatformTestUserPassword =
                            MakePasswordBox(new GUIContent(passwdLabel, passwdHint),
                                StandalonePlatformSettings.OculusPlatformTestUserPassword);
                    ...
                    {
                        var loggedInMsg = "Currently using the credentials associated with " +
                                          StandalonePlatformSettings.OculusPlatformTestUserEmail;
                        EditorGUILayout.HelpBox(loggedInMsg, UnityEditor.MessageType.Info);
          ...
          {
            EditorGUILayout.HelpBox("VR Support isn't enabled in the Player Settings", UnityEditor.MessageType.Warning);
          }

          PlayerSettings.virtualRealitySupported =
 MakeToggle(new GUIContent("Virtual Reality Support"), PlayerSettings.virtualRealitySupported);
#endif
#endif

                    PlayerSettings.bundleVersion =
                        MakeTextBox(new GUIContent("Bundle Version"), PlayerSettings.bundleVersion);
#if UNITY_5_3 || UNITY_5_4 || UNITY_5_5
          PlayerSettings.bundleIdentifier =
 MakeTextBox(new GUIContent("Bundle Identifier"), PlayerSettings.bundleIdentifier);
#else
        ...
        private void GetAccessToken()
        {
            if (getAccessTokenRequest != null && getAccessTokenRequest.isDone)
            {
                ...
                {
                    var Response =
                        JsonUtility.FromJson<OculusStandalonePlatformResponse>(getAccessTokenRequest.downloadHandler
                            .text);
        ...
        private static string GetPlatformPluginPath(PluginPlatform platform)
        {
            ...
            switch (platform)
            {
                ...
                case PluginPlatform.Android64:
                    ...
                    break;
                default:
                    throw new ArgumentException("Attempted to enable platform support for unsupported platform: " +
                                                platform);
            }
